#!/usr/bin/env python3
"""
Clinical Decision Pipeline Sweep Script for Azure OpenAI

Runs the clinical decision pipeline using Azure OpenAI (GPT-4o) across multiple seeds and input files.
...
def run_sweep(
    ...
    """
    Run clinical decision pipeline sweep with Azure OpenAI.
    ...
    print("=" * 80)
    print("CLINICAL DECISION PIPELINE SWEEP (Azure OpenAI)")
    print("=" * 80)
    print(f"Model: {model}")
    print(f"Seeds: {seeds}")
    ...
    total_jobs = len(seeds) * len(csv_files)
    ...
    for seed, csv_file in product(seeds, csv_files):
        job_count += 1
...
def main():
    parser = argparse.ArgumentParser(
        description='Run clinical decision pipeline sweep using Azure OpenAI'
    )
    parser.add_argument(
        '--model',
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=[0, 1, 2],
        help='List of seeds (default: 0 1 2)'
    )
    parser.add_argument(
        '--data-dir',
        type=str,
